...
const loadSettings = async () => {
    ...
    try {
        const [profileResponse, companyResponse, systemResponse] = await Promise.all([
            Axios.get(API_CONFIG.IDENTITY.PROFILE_ME),
            Axios.get(API_CONFIG.IDENTITY.PROFILE_COMPANY),
            Axios.get(API_CONFIG.IDENTITY.PROFILE_SYSTEM)
        ]);
...
const saveProfile = async () => {
    loading.value = true;
    try {
        const response = await Axios.patch(API_CONFIG.IDENTITY.PROFILE_ME, {
...
const saveCompany = async () => {
    loading.value = true;
    try {
        const response = await Axios.patch(API_CONFIG.IDENTITY.PROFILE_COMPANY, {
